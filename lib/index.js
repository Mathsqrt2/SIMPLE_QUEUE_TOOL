...
const appInit = () => {
    ...
    adobeApp.evalScript(`isItFirstUseJSX('${adobeApp.getSystemPath(SystemPath.EXTENSION)}')`, function (res) {
        const data = JSON.parse(res);
        if (data.isItFirstUse) {
            adobeApp.openURLInDefaultBrowser("https://mbugajski.pl/plugins/simple-queue-tool-use-guide");
        }
    })
    adobeApp.evalScript(`loadConfiguration('${adobeApp.getSystemPath(SystemPath.EXTENSION)}')`, function (res) {
        if (res) {
