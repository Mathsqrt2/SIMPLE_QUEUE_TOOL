...
const loadValues = () => {
    cs.evalScript(`loadConfiguration('${cs.getSystemPath(SystemPath.EXTENSION)}')`, function (res) {
        if (res) {
            ...
            const secureModeBox = document.getElementById("secure_mode");
            const bonusOption = document.getElementById("bonusOption");
            const renderSwitch = document.getElementById("renderSwitch");
            ...
            if (config.type == "sequences") {
                ...
                switch (config.basedOn) {
                    ...
                    case "each clip audio":
                        basedOn.selectedIndex = 3;
                        break;
                    case "clip audio on track":
                        basedOn.selectedIndex = 4;
                        baseIndexBox.classList.remove("hidden");
                        baseIndex.value = config.basedOnIndex;
                        break;
            ...
            if (config.secureMode) {
                secureModeSwitch.selectedIndex = 0;
            } else {
                secureModeSwitch.selectedIndex = 0;
            } 
            if(config.renderAfterQueue){
                renderSwitch.selectedIndex = 0;
            } else {
                renderSwitch.selectedIndex = 1;
            }
        }
    });
}
const attachJSXListener = () => {
...
addEventListener("load", attachJSXListener);
